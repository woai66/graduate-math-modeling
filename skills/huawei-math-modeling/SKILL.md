...
`templates/paper-skeleton.docx` 是可直接填写的论文骨架：**封面与摘要页直接沿用官方附件3 模板**（官方 logo、报名表格、华文新魏/隶书标题、页边距、页码起始 0），正文按附件2 的字体字号与优秀论文范式注入，并含一个真正的 TOC 域目录（一级中文序号加粗、二级 N.M 缩进、点线引导、页码右对齐）。已用 Microsoft Word 打开、更新域并导出 PDF 完成原生渲染验证（封面 0、摘要页 1、目录 2—3、正文第 4 页起，无空白页）。用法见 [骨架使用说明](templates/paper-skeleton-usage.md)。

改稿后在 Word 中 `Ctrl+A` → `F9` 更新目录与页码；手工敲目录会在增删章节后失准，因此一律用域。

身份信息的正确边界：封面按官方模板填写学校/参赛队号/队员姓名；正文及其后各页不得出现任何可能显示答题人身份的标志。
