...
def main() -> None:
    ...
    subtitle.add_run("基于 NumPy 与 Pillow 的可复现基线实验报告").italic = True
    doc.add_paragraph()

    doc.add_heading("摘要", level=1)
    paragraph(doc, f"针对钻孔成像展开图中的裂隙识别、参数表征、粗糙度评价和多钻孔连通性分析问题，本文建立了一套不依赖深度学习框架的可复现基线流程。问题一使用图像梯度和分位数阈值生成像素边缘掩膜，在 10 张附件 1 图像上的平均边缘覆盖率为 {q1_coverage:.2%}，下采样连通域数量均值为 {q1_components:.1f}。问题二将每列最暗像素作为粗略观测点，利用线性化正弦模型估计振幅、周期、相位和中心线位置，10 张图像的拟合决定系数 R² 范围为 {min(q2_r2):.4f}—{max(q2_r2):.4f}，说明简单暗像素基线不足以直接完成可靠裂隙拟合。问题三基于梯度轮廓进行等间距采样，发现样本数从 32 增至 256 时 JRC 估计由 {q3_values[0]:.2f} 变为 {q3_values[-1]:.2f}，表明轮廓提取和采样策略是粗糙度评价的主要敏感因素。问题四以钻孔分段的边缘密度差异构造启发式连通评分，并生成六孔三维投影示意图；该评分用于候选补孔排序，不应解释为经过现场标定的真实概率。本文的主要价值是建立数据、代码、结果表和 Word 论文之间的可追溯链路，并明确了后续 U-Net、DBSCAN、RANSAC 和概率图模型的增强方向。")
    ...
    paragraph(doc, "设灰度图像为 I(x,y)，使用中心差分近似水平和竖直梯度，定义边缘强度 E(x,y) = sqrt(Gx(x,y)^2 + Gy(x,y)^2)。对每张图像取 E 的 90% 分位数作为阈值 τ，得到基线掩膜 M(x,y) = 1[E(x,y) ≥ τ]。该策略不需要标签，适合验证数据链路，但会同时保留岩石纹理和钻进痕迹。")
    paragraph(doc, f"10 张图像的平均边缘覆盖率为 {q1_coverage:.2%}，下采样后连通域均值为 {q1_components:.1f}。覆盖率接近固定的 10% 是分位数阈值直接造成的统计性质，不能解释为裂隙面积比例；因此该结果只用于检查输入、输出尺寸和后续流程。")
    ...
    q2_explanation = paragraph(doc, "题目给出的正弦模型为 y = R sin(2πx/P + β) + C。基线将每一列灰度最小的像素作为观测点，并将周期 P 固定为图像宽度 244 像素。利用 sin(a+b) 展开后，模型可以写成 y = a sin(2πx/P) + b cos(2πx/P) + C，再使用最小二乘求解 a、b、C，最后得到 R = sqrt(a²+b²) 和 β = atan2(b,a)。")
    ...
    paragraph(doc, "优点方面，本文基线不依赖 OpenCV、SciPy 或深度学习框架，能够在轻量环境下完成数据读取、四问串联、结果表生成和图表输出；每个结果都由脚本生成，便于复现。问题一验证了二值输出尺寸和边缘覆盖统计，问题二和问题三的低质量/高敏感性结果及时暴露了简单模型的不足，避免把错误结果包装成最终结论。")
