...
用户可能已经手工编辑过模板、论文或配置。任何整文件覆盖（复制、重新生成、批量替换）之前：

1. 对目标文件与基准文件做**完整**差异比对，必须同时比较段落文本、段落样式、样式表、
   节设置与表格，不能只比文字，也不能用会截断输出的比较；
2. 差异结果要完整查看，不能只看前几十行就下结论；
3. 无法完整比对（例如文件被 Word 占用而读不出）时，**不要覆盖**：改用原地局部修改，
   或先备份到 `*.bak-YYYYMMDD-HHMM` 再操作；
4. 覆盖后向用户报告"改了哪些、保留了什么"，让用户能核对。

2026-09-22 曾因比对不完整（只比段落文字、且带样式的比对被截断）覆盖了用户手工改过的
附录结构，造成返工。此类失误会直接毁掉用户工作，必须避免。
